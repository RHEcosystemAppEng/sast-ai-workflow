...
class StratifiedSplitter:
    ...
    def discover_package_pairs(self, exclusion_list: Optional[set] = None) -> Dict[str, Dict]:
        ...
        Returns dict mapping package_name to {
        ...
        for excel_path in excel_files:
            pkg_name = self._extract_package_name(excel_path)
            if pkg_name not in pairs:
                pairs[pkg_name] = {'excel_path': None, 'ignore_err_path': None}
            pairs[pkg_name]['excel_path'] = excel_path
        ...
        if processed_known_non_issues_dir.exists():
            for ignore_err_file in processed_known_non_issues_dir.glob("*_ignore.err"):
                nvr = ignore_err_file.name.replace('_ignore.err', '')

                try:
                    from Utils.rpm_utils import parse_nvr
                    nvr_info = parse_nvr(nvr + ".xlsx")
                    pkg_name = nvr_info['package']

                    if pkg_name not in pairs:
                        pairs[pkg_name] = {'excel_path': None, 'ignore_err_path': None}
                    pairs[pkg_name]['ignore_err_path'] = ignore_err_file
                    logger.debug(f"Discovered NVR-enriched ignore.err: {pkg_name} -> {nvr}")
                except Exception as e:
                    logger.warning(f"Failed to parse NVR from {ignore_err_file.name}: {e}")
                    continue
        else:
    ...
    def load_and_analyze_packages(self, exclusion_list: Optional[set] = None):
        ...
        logger.info(f"Found {len(package_pairs)} unique packages")
        ...
        for pkg_name, files in package_pairs.items():
            ...
            if excel_path and ignore_err_path:
                ...
                logger.warning(f"Package {pkg_name} has neither Excel nor ignore.err, skipping")
                continue

            metadata = self.compute_package_metadata_with_pairing(
                pkg_name, excel_path, ignore_err_path, pairing_status
            )
        ...
        logger.info(f"  Ignore.err only: {ignore_err_only_count}")
        ...
        def write_manifest(packages, manifest_path):
            with open(manifest_path, 'w') as f:
                ...
                for pkg in sorted(packages, key=lambda p: p.package_name):
                    if pkg.has_ignore_err:
                        pairing_status = "paired"
                    else:
                        pairing_status = "excel_only"
                    f.write(f"{pkg.package_name} [{pairing_status}]\n")
