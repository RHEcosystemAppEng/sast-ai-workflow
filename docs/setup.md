...
| LLM_CONTEXT_WINDOW             | 32768                                     |               | 65536                                      | Model's total context window size in tokens. Used to dynamically calculate max output tokens per request, preventing context overflow errors. Set this to match your vLLM deployment's `--max-model-len`. |
