...
from sast_agent_workflow.nodes.data_fetcher import DataFetcherConfig, data_fetcher
...
class TestDataFetcherCore(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_preserves_tracker_structure_and_populates_source_code(
        self, mock_repo_handler_factory
    ):
        """Given a sample tracker, data_fetcher preserves tracker structure.
        ...
        result_tracker = await TestUtils.run_single_fn(
            data_fetcher, self.data_fetcher_config, self.builder, self.sample_tracker
        )
    ...
    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_initial_iteration_populates_source_code_from_error_trace(
        self, mock_repo_handler_factory
    ):
        ...
        # testing
        result_tracker = await TestUtils.run_single_fn(
            data_fetcher, self.data_fetcher_config, self.builder, tracker
        )
        ...
        for per_issue_data in result_tracker.issues.values():
            ...
            self.assertTrue(
                any("int main()" in snippet for snippet in per_issue_data.source_code["file.c"])
            )
        self.assertEqual(
            mock_repo_handler.get_source_code_blocks_from_error_trace.call_count,
            len(tracker.issues),
        )
    ...
    async def test_skips_issue_marked_final(self, mock_repo_handler_factory):
        ...
        new_file_counts = sum(
            1 for p in result_tracker.issues.values() if "file.c" in p.source_code
        )
        self.assertEqual(new_file_counts, 1)

    @patch(
        "sast_agent_workflow.nodes.data_fetcher.repo_handler_factory",
    ...
    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_subsequent_iteration_fetches_by_instructions_and_merges(
        self, mock_repo_handler_factory
    ):
        """Subsequent iteration fetches additional data per instructions."""
        # preparation
        ...
        for per_issue_data in tracker.issues.values():
            per_issue_data.analysis_response = TestUtils.create_sample_analysis_response(
                ...
                instructions=[
                    InstructionResponse(
                        expression_name="foo",
                        referring_source_code_path="src/foo.c",
        ...
        for per_issue_data in result_tracker.issues.values():
            ...
            self.assertTrue(
                any("int foo()" in snippet for snippet in per_issue_data.source_code["src/foo.c"])
            )
            self.assertTrue(
                any(
                    "#define BAR" in snippet
                    for snippet in per_issue_data.source_code["include/bar.h"]
                )
            )
            self.assertEqual(per_issue_data.analysis_response.is_final, FinalStatus.FALSE.value)

    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_subsequent_iteration_when_second_analysis_not_needed_then_no_fetch(
        self, mock_repo_handler_factory
    ):
    ...
    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_malformed_missing_source_codes_yield_no_additions_and_set_is_final(
        self, mock_repo_handler_factory
    ):
        """Malformed or empty missing_source_codes should not add source
        and should set is_final to TRUE when instructions exist."""
    ...
    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_subsequent_iteration_with_no_analysis_response_skips(
        self, mock_repo_handler_factory
    ):
        tracker = TestUtils.create_sample_tracker(self.sample_issues, iteration_count=1)
    ...
    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_subsequent_iteration_with_no_repo_handler_continues(
        self, mock_repo_handler_factory
    ):
        tracker = TestUtils.create_sample_tracker(self.sample_issues, iteration_count=1)
        ...
        for per_issue in tracker.issues.values():
            per_issue.analysis_response = TestUtils.create_sample_analysis_response(
                ...
                instructions=[
                    InstructionResponse(
                        expression_name="foo",
                        referring_source_code_path="a.c",
                        recommendation="inspect",
                    )
                ],
            )
    ...
    @patch("sast_agent_workflow.nodes.data_fetcher.repo_handler_factory")
    async def test_subsequent_iteration_merge_appends_to_existing_path(
        self, mock_repo_handler_factory
    ):
        tracker = TestUtils.create_sample_tracker(self.sample_issues, iteration_count=1)
        for per_issue in tracker.issues.values():
        ...
        missing = "code of src/foo.c file:\nint foo(){return 1;}\n"
        mock_repo_handler = Mock()
        mock_repo_handler.extract_missing_functions_or_macros.return_value = (missing, set())
        mock_repo_handler_factory.return_value = mock_repo_handler
        result_tracker = await TestUtils.run_single_fn(
