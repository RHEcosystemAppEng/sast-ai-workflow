...
class TestWriteResultsCore(unittest.IsolatedAsyncioTestCase):
    ...
    async def test__write_results__final_issues_writes_to_excel_successfully(self):
        ...
        with (
            ...
            result_tracker = await TestUtils.run_single_fn(
                write_results, self.write_results_config, self.builder, tracker
            )
            ...
            # Verify writer called with correct parameters - using metrics-based evaluation summary
    ...
    async def test__write_results__preserves_all_tracker_data_unchanged(self):
        ...
        original_tracker = TestUtils.create_sample_tracker(
            issues_dict=per_issue_data, config=self.mock_config
        )
        ...
        with (
            ...
            # testing
            result_tracker = await TestUtils.run_single_fn(
                write_results, self.write_results_config, self.builder, original_tracker
            )
    ...
    async def test__write_results__empty_tracker_handles_gracefully(self):
        ...
        with (
            ...
            # testing
            result_tracker = await TestUtils.run_single_fn(
                write_results, self.write_results_config, self.builder, empty_tracker
            )
    ...
    async def test__write_results__excel_writer_failure_handles_gracefully(self):
        ...
        with (
            ...
            mock_writer.side_effect = Exception("Excel writing failed")
    ...
    async def test__write_results__invalid_metrics_triggers_fallback_evaluation_summary_creation(
        self,
    ):
        # preparation
        issues = [
    ...
    async def test__write_results__missing_ml_performance_metrics_triggers_fallback_gracefully(
        self,
    ):
        """
    ...
    async def test__write_results__evaluation_summary_integration_with_real_excel_writer(self):
        ...
        try:
            ...
            self.mock_config.INPUT_REPORT_FILE_PATH = (
                "test_report.html"  # Not a URL, so no Google Sheets
            )
            ...
            with patch(
                ...
                try:
                    ...
                    self.assertGreater(
                        os.path.getsize(temp_excel_path), 1000
