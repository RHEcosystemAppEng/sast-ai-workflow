...
from sast_agent_workflow.nodes.judge_llm_analysis import (
    JudgeLLMAnalysisConfig,
    judge_llm_analysis,
)
...
class TestJudgeLLMAnalysisCore(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.IssueAnalysisService")
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.VectorStoreService")
    async def test_processes_non_final_issues(
        self, mock_vector_service_class, mock_issue_analysis_service_class
    ):
        ...
        mock_issue_analysis_service.analyze_issue_core_only.return_value = (
            "test prompt",
            mock_llm_response,
        )
        ...
        result_tracker = await TestUtils.run_single_fn(
            judge_llm_analysis, self.judge_llm_analysis_config, self.builder, self.sample_tracker
        )

        # Verification
        # Services should be initialized correctly
        mock_vector_service_class.assert_called_once()
        mock_issue_analysis_service_class.assert_called_once_with(
            self.mock_config, mock_vector_service
        )
        ...
        self.assertEqual(
            result_tracker.issues[issue_ids[1]].analysis_response.is_final, FinalStatus.FALSE.value
        )
        # Check that basic fields are populated
        self.assertEqual(
            result_tracker.issues[issue_ids[1]].analysis_response.prompt, "test prompt"
        )
        self.assertEqual(
            result_tracker.issues[issue_ids[1]].analysis_response.justifications,
            ["test justification"],
        )
        ...
        self.assertEqual(
            result_tracker.issues[issue_ids[1]].analysis_response.short_justifications, ""
        )
    ...
    async def test_raises_error_when_no_config(self):
        ...
        with self.assertRaises(ValidationError) as context:
            await TestUtils.run_single_fn(
                judge_llm_analysis,
                self.judge_llm_analysis_config,
                self.builder,
                self.sample_tracker,
            )
    ...
    async def test_raises_error_when_tracker_none(self):
        ...
        with self.assertRaises(ValueError) as context:
            await TestUtils.run_single_fn(
                judge_llm_analysis, self.judge_llm_analysis_config, self.builder, None
            )
    ...
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.IssueAnalysisService")
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.VectorStoreService")
    async def test_raises_error_when_service_init_fails(
        self, mock_vector_service_class, mock_issue_analysis_service_class
    ):
    ...
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.IssueAnalysisService")
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.VectorStoreService")
    async def test_continues_processing_on_analysis_failure(
        self, mock_vector_service_class, mock_issue_analysis_service_class
    ):
        """LLM analysis failure for one issue does not stop processing
        of remaining issues."""
    ...
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.IssueAnalysisService")
    @patch("sast_agent_workflow.nodes.judge_llm_analysis.VectorStoreService")
    async def test_skips_invalid_issue_data(
        self, mock_vector_service_class, mock_issue_analysis_service_class
    ):
