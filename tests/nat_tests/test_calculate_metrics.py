...
from sast_agent_workflow.nodes.calculate_metrics import (
...
class TestCalculateMetricsCore(unittest.IsolatedAsyncioTestCase):
    ...
    async def test__nat_tests__ragas_metrics_disabled_still_calculates(self):
        ...
        tracker.issues[issue_ids[0]].analysis_response.investigation_result = (
            CVEValidationStatus.FALSE_POSITIVE.value
        )

        # testing
        with patch(
            "sast_agent_workflow.nodes.calculate_metrics.get_human_verified_results"
        ) as mock_get_ground_truth:
            mock_get_ground_truth.return_value = None
            result_tracker = await TestUtils.run_single_fn(
                calculate_metrics, self.calculate_metrics_config, self.builder, tracker
            )
    ...
    async def test__nat_tests__no_config_preserves_empty_metrics(self):
        ...
        # testing
        with self.assertLogs("sast_agent_workflow.nodes.calculate_metrics", level="WARNING") as log:
            result_tracker = await TestUtils.run_single_fn(
                calculate_metrics, self.calculate_metrics_config, self.builder, tracker
            )
    ...
    async def test__nat_tests__no_completed_issues_when_include_non_final_is_false_returns_error(
        self,
    ):
        # preparation
        tracker = TestUtils.create_sample_tracker(self.sample_issues)
        tracker.config = self._create_mock_config(
            calculate_ragas_metrics=True, write_results_include_non_final=False
        )
    ...
    async def test__nat_tests__mixed_issues_when_include_non_final_is_false_processes_only_final(
        self,
    ):
        # preparation
        tracker = TestUtils.create_sample_tracker(self.sample_issues)
        ...
        tracker.issues[issue_ids[1]].analysis_response.investigation_result = (
            CVEValidationStatus.TRUE_POSITIVE.value
        )
    ...
    @patch("sast_agent_workflow.nodes.calculate_metrics.get_human_verified_results")
    async def test__nat_tests__no_ground_truth_calculates_basic_metrics(
        self, mock_get_ground_truth
    ):
        # preparation
        mock_get_ground_truth.return_value = None
    ...
    @patch("sast_agent_workflow.nodes.calculate_metrics.get_human_verified_results")
    async def test__nat_tests__with_ground_truth_calculates_full_metrics(
        self, mock_get_ground_truth
    ):
        # preparation
        mock_ground_truth = {"def1": "yes", "def2": "no"}
    ...
    @patch("sast_agent_workflow.nodes.calculate_metrics.get_human_verified_results")
    async def test__nat_tests__perfect_predictions_calculates_correct_metrics(
        self, mock_get_ground_truth
    ):
        # preparation
        mock_ground_truth = {"def1": "no", "def2": "no"}
        mock_get_ground_truth.return_value = mock_ground_truth
        ...
        for per_issue_data in tracker.issues.values():
            per_issue_data.analysis_response.investigation_result = (
                CVEValidationStatus.TRUE_POSITIVE.value
            )
    ...
    async def test__nat_tests__critique_enabled_uses_correct_config(self):
        ...
        tracker = TestUtils.create_sample_tracker(self.sample_issues)
        tracker.config = self._create_mock_config(
            calculate_ragas_metrics=True, use_critique_as_final=True
        )
        ...
        with patch(
            ...
            with patch(
                "sast_agent_workflow.nodes.calculate_metrics.EvaluationSummary"
            ) as mock_eval_summary:
                mock_get_ground_truth.return_value = None
                mock_eval_instance = Mock()
                mock_eval_instance.predicted_summary = [
                    (issue_ids[0], per_issue.analysis_response, 0)
                ]
