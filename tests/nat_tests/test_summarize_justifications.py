...
from sast_agent_workflow.nodes.summarize_justifications import (
    SummarizeJustificationsConfig,
...
class TestSummarizeJustificationsCore(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_final_issues_with_detailed_justifications_populates_short(
        self,
    ):
        # preparation
        issues = [
        ...
        with patch(
            "sast_agent_workflow.nodes.summarize_justifications.IssueAnalysisService"
            ...
            for issue_id in ["final_issue_1", "final_issue_2"]:
                self.assertEqual(
                    result_tracker.issues[issue_id].analysis_response.short_justifications,
                    "Summarized: Buffer overflow detected",
                )
                self.assertEqual(
                    result_tracker.issues[issue_id].analysis_response.justifications,
                    ["Detailed justification 1", "Detailed justification 2"],
                )

            self.assertEqual(mock_service.summarize_justification.call_count, 2)

    async def test_final_issues_with_existing_short_preserves_summaries(
        self,
    ):
        # preparation
        issues = [
            TestUtils.create_sample_issue(
                issue_id="already_processed", issue_type="BUFFER_OVERFLOW"
            )
        ]
        ...
        with patch(
            ...
            # assertion
            self.assertEqual(
                result_tracker.issues["already_processed"].analysis_response.short_justifications,
                "Already summarized content",
            )
            mock_service.summarize_justification.assert_not_called()

    async def test__summarize_justifications__mixed_issue_states_processes_only_eligible_issues(
        self,
    ):
        # preparation
        issues = [
            ...
            TestUtils.create_sample_issue(
                issue_id="already_processed_issue", issue_type="MEMORY_LEAK"
            ),
            TestUtils.create_sample_issue(
                issue_id="no_analysis_response", issue_type="NULL_POINTER"
            ),
        ]
        ...
        per_issue_data["already_processed_issue"].analysis_response.short_justifications = (
            "Already done"
        )
        ...
        with patch(
            ...
            # assertion
            self.assertEqual(
                result_tracker.issues["eligible_issue"].analysis_response.short_justifications,
                "New summary for eligible issue",
            )
            self.assertEqual(
                result_tracker.issues["non_final_issue"].analysis_response.short_justifications,
                "New summary for eligible issue",
            )
            self.assertEqual(
                result_tracker.issues[
                    "already_processed_issue"
                ].analysis_response.short_justifications,
                "Already done",
            )
    ...
    async def test__summarize_justifications__summarization_error_handles_gracefully(self):
        ...
        with patch(
            ...
            self.assertIsInstance(result_tracker, SASTWorkflowTracker)
            self.assertEqual(
                result_tracker.issues["error_issue"].analysis_response.short_justifications,
                DEFAULT_FIELD_VALUE,
            )
            self.assertEqual(
                result_tracker.issues["error_issue"].analysis_response.justifications,
