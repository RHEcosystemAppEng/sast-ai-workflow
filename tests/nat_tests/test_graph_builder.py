...
@pytest.mark.skip(reason="should_continue_analysis removed - agent handles iteration internally")
class TestShouldContinueAnalysis:
    """Test the conditional edge logic for workflow routing.

    NOTE: These tests are skipped because should_continue_analysis was removed
    as part of the agent-based investigation refactor. The agent now handles
    iteration logic internally via circuit breakers and comprehensive_evaluation.
    """

    def test__should_continue_analysis__issues_needing_second_analysis_under_limit_returns_data_fetcher(
        self,
    ):
        # Preparation
        ...
        tracker = SASTWorkflowTracker(  # noqa: F841
            issues=issues_dict, config=config, iteration_count=1  # Under limit
        )

        # Testing
        # # result = should_continue_analysis(tracker)  # Removed
        result = None  # Placeholder for skipped test  # Removed - see class docstring
        result = None  # noqa: F841 - Placeholder for skipped test

        # Assertion
        # assert result == WorkflowNode.DATA_FETCHER.value  # Skipped
        assert True  # Placeholder assertion

    def test__should_continue_analysis__no_issues_needing_second_analysis_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=2)
        issues_dict = TestUtils.create_sample_per_issue_data_dict(
            issues, is_final=FinalStatus.TRUE.value  # All final issues - no second analysis needed
        )
        config = Mock(spec=Config)
        config.MAX_ANALYSIS_ITERATIONS = 3
        tracker = SASTWorkflowTracker(issues=issues_dict, config=config, iteration_count=1)
        ...
        # Assertion
        # assert result == WorkflowNode.SUMMARIZE_JUSTIFICATIONS.value  # Skipped
        assert True  # Placeholder assertion

    def test__should_continue_analysis__issues_needing_second_analysis_at_limit_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=2)
        ...
        tracker = SASTWorkflowTracker(  # noqa: F841
            issues=issues_dict, config=config, iteration_count=2  # At limit
        )
    ...
    def test__should_continue_analysis__issues_needing_second_analysis_over_limit_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=2)
        ...
        config.MAX_ANALYSIS_ITERATIONS = 2
        tracker = SASTWorkflowTracker(  # noqa: F841
            issues=issues_dict, config=config, iteration_count=3  # Over limit
        )
    ...
    def test__should_continue_analysis__config_missing_max_iterations_uses_default_returns_data_fetcher(
        self,
    ):
        # Preparation
        ...
        tracker = SASTWorkflowTracker(  # noqa: F841
            issues=issues_dict, config=None, iteration_count=1  # Under default limit of 2
        )
    ...
    def test__should_continue_analysis__config_missing_max_iterations_at_default_limit_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=1)
        ...
        tracker = SASTWorkflowTracker(  # noqa: F841
            issues=issues_dict, config=None, iteration_count=2  # At limit of default (2)
        )
    ...
    def test__should_continue_analysis__iteration_count_missing_sets_to_1_and_returns_data_fetcher(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=1)
        ...
        config = Mock(spec=Config)
        config.MAX_ANALYSIS_ITERATIONS = 2
        tracker = SASTWorkflowTracker(issues=issues_dict, config=config)
    ...
    def test__should_continue_analysis__iteration_count_missing_sets_to_1_and_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=1)
    ...
    def test__should_continue_analysis__non_final_issues_without_instructions_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=2)
    ...
    def test__should_continue_analysis__mixed_issues_some_needing_second_analysis_returns_data_fetcher(
        self,
    ):
        # Preparation
    ...
    def test__should_continue_analysis__empty_issues_dict_returns_summarize_justifications(self):
        ...
        config = Mock(spec=Config)
        config.MAX_ANALYSIS_ITERATIONS = 3
        tracker = SASTWorkflowTracker(issues={}, config=config, iteration_count=1)  # Empty issues
    ...
    def test__should_continue_analysis__issues_without_analysis_response_returns_summarize_justifications(
        self,
    ):
        # Preparation
        issues = TestUtils.create_sample_issues(count=2)
        ...
        for issue in issues:
            issues_dict[issue.id] = PerIssueData(
                issue=issue, analysis_response=None  # No analysis response
            )
        config = Mock(spec=Config)
        config.MAX_ANALYSIS_ITERATIONS = 3
        tracker = SASTWorkflowTracker(issues=issues_dict, config=config, iteration_count=1)
    ...
    def test__should_continue_analysis__non_final_false_positive_issues_returns_summarize_justifications(
        self,
    ):
...
class TestGraphBuilder:
    ...
    NOTE: The graph structure has changed from the 3-iteration loop to agent-based
    investigation. The new signature is:
    - pre_process_node, filter_node, investigate_issue_node,
      summarize_justifications_node, calculate_metrics_node, write_results_node
    """
    ...
    def mock_nodes(self):
        ...
        return {
            ...
            "investigate_issue_node": AsyncMock(),
    ...
    def test__build_sast_workflow_graph__contains_all_expected_nodes(self, mock_nodes):
        # Preparation - agent-based workflow nodes
        expected_nodes = [
            WorkflowNode.PRE_PROCESS.value,
            WorkflowNode.FILTER.value,
            WorkflowNode.INVESTIGATE_ISSUE.value,
    ...
    def test__build_sast_workflow_graph__has_linear_edges_between_sequential_nodes(
        ...
        # Preparation - the new workflow is fully linear (no conditional edges)
        expected_linear_edges = {
            ("__start__", WorkflowNode.PRE_PROCESS.value),
            (WorkflowNode.PRE_PROCESS.value, WorkflowNode.FILTER.value),
            (WorkflowNode.FILTER.value, WorkflowNode.INVESTIGATE_ISSUE.value),
            ...
            (WorkflowNode.WRITE_RESULTS.value, "__end__"),
        }
        ...
        actual_edges_tuples = set((edge.source, edge.target) for edge in actual_edges)
    ...
    def test__build_sast_workflow_graph__none_node_function_raises_error(self):
        # Preparation - agent-based workflow signature
        nodes_with_none = {
            "pre_process_node": AsyncMock(),
