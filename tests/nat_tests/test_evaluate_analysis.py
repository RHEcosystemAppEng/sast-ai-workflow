...
from dto.ResponseStructures import InstructionResponse, RecommendationsResponse
from dto.SASTWorkflowModels import PerIssueData, SASTWorkflowTracker
from sast_agent_workflow.nodes.evaluate_analysis import (
    EvaluateAnalysisConfig,
    evaluate_analysis,
)
...
class TestEvaluateAnalysisCore(unittest.IsolatedAsyncioTestCase):
    ...
    async def test__evaluate_analysis__non_final_issues_calls_recommend_and_updates_response(self):
        # Preparation
        issues = [
            TestUtils.create_sample_issue(
                issue_id="non_final_issue_1", issue_type="BUFFER_OVERFLOW"
            ),
            TestUtils.create_sample_issue(
                issue_id="non_final_issue_2", issue_type="USE_AFTER_FREE"
            ),
        ]
        ...
        tracker = TestUtils.create_sample_tracker(
            issues_dict=per_issue_data, config=self.mock_config
        )
        ...
        with (
            patch(
                "sast_agent_workflow.nodes.evaluate_analysis.VectorStoreService"
            ) as mock_vector_service_class,
            patch(
                "sast_agent_workflow.nodes.evaluate_analysis.IssueAnalysisService"
            ) as mock_analysis_service_class,
        ):
            ...
            result_tracker = await TestUtils.run_single_fn(
                evaluate_analysis, self.evaluate_analysis_config, self.builder, tracker
            )
            ...
            for issue_id in ["non_final_issue_1", "non_final_issue_2"]:
                ...
                self.assertEqual(
                    analysis_response.recommendations, mock_recommendations_response.recommendations
                )
                self.assertEqual(
                    analysis_response.evaluation, mock_recommendations_response.justifications
                )
    ...
    async def test__evaluate_analysis__final_issues_skips_processing(self):
        ...
        per_issue_data = TestUtils.create_sample_per_issue_data_dict(
            issues, is_final=FinalStatus.TRUE.value  # Already final
        )
    ...
    async def test__evaluate_analysis__mixed_issues_processes_only_non_final(self):
        ...
        per_issue_data["final_issue"] = PerIssueData(
            ...
            analysis_response=TestUtils.create_sample_analysis_response(
                is_final=FinalStatus.TRUE.value, justifications=["Final issue justification"]
            ),
        )
        per_issue_data["non_final_issue"] = PerIssueData(
            issue=issues[1],
            analysis_response=TestUtils.create_sample_analysis_response(
                is_final=FinalStatus.FALSE.value, justifications=["Non-final issue justification"]
            ),
        )
        per_issue_data["no_analysis_issue"] = PerIssueData(
            issue=issues[2], analysis_response=None  # No analysis response
        )
        ...
        with (
            ...
            self.assertEqual(
                result_tracker.issues["final_issue"].analysis_response.is_final,
                FinalStatus.TRUE.value,
            )

            # Verify non-final issue was processed
            self.assertEqual(
                result_tracker.issues["non_final_issue"].analysis_response.is_final,
                FinalStatus.TRUE.value,
            )
    ...
    async def test__evaluate_analysis__max_iterations_reached_skips_processing(self):
        ...
        per_issue_data = TestUtils.create_sample_per_issue_data_dict(
            issues, is_final=FinalStatus.FALSE.value
        )
        ...
        with (
            ...
            self.assertEqual(
                result_tracker.issues["test_issue"].analysis_response.is_final,
                FinalStatus.FALSE.value,
            )
    ...
    async def test__evaluate_analysis__no_config_raises_validation_error(self):
        ...
        tracker = TestUtils.create_sample_tracker(issues_dict=per_issue_data)
    ...
    async def test__evaluate_analysis__empty_issues_raises_validation_error(self):
        ...
        with self.assertRaises(ValidationError):
            await TestUtils.run_single_fn(
                evaluate_analysis, self.evaluate_analysis_config, self.builder, empty_tracker
            )
    ...
    async def test__evaluate_analysis__llm_service_error_handles_gracefully(self):
        ...
        per_issue_data = TestUtils.create_sample_per_issue_data_dict(
            issues, is_final=FinalStatus.FALSE.value, justifications=[DEFAULT_FIELD_VALUE]
        )
    ...
    async def test__evaluate_analysis__non_final_becomes_final_updates_correctly(self):
        # Preparation
        issues = [
            TestUtils.create_sample_issue(issue_id="becoming_final", issue_type="BUFFER_OVERFLOW")
        ]
