...
├── benchmark-config.yaml            # ConfigMap (generated by make eventlistener)
├── benchmark-config.yaml.example    # Template (optional reference)
...
**Note:** `benchmark-config.yaml` is automatically generated when you run `make eventlistener` with the required parameters.
...
### Step 2: Find Your Orchestrator URL

```bash
# Find orchestrator service
oc get svc -l app=sast-ai-orchestrator -n your-namespace

# Typical format:
# http://<service-name>.<namespace>.svc.cluster.local:<port>
# Example: http://sast-ai-orchestrator.sast-ai.svc.cluster.local:8080
```

### Step 3: Deploy EventListener

Deploy the EventListener with required parameters:
...
**Required Parameters:**
- `ORCHESTRATOR_API_URL` - Orchestrator service URL (cluster-internal)
...
# Update if needed - regenerate with new parameters
cd deploy
make eventlistener \
  ORCHESTRATOR_API_URL=<updated-url> \
  NAMESPACE=your-namespace
```
...
|-----|-------------|---------|
| `orchestrator-api-url` | Base URL of orchestrator service | `http://sast-ai-orchestrator.sast-ai.svc.cluster.local:8080` |
...
# Find orchestrator URL
ORCH_URL=$(oc get svc -l app=sast-ai-orchestrator -n sast-ai-benchmark -o jsonpath='{.items[0].metadata.name}')
ORCH_PORT=$(oc get svc -l app=sast-ai-orchestrator -n sast-ai-benchmark -o jsonpath='{.items[0].spec.ports[0].port}')

# Deploy MLOps pipeline overlay
cd deploy
make tasks ENV=mlop NAMESPACE=sast-ai-benchmark

# Deploy EventListener
make eventlistener \
  ORCHESTRATOR_API_URL=http://${ORCH_URL}.sast-ai-benchmark.svc.cluster.local:${ORCH_PORT} \
  NAMESPACE=sast-ai-benchmark
```
...
1. **Deploy** with your parameters:
   ```bash
   make eventlistener \
     ORCHESTRATOR_API_URL=<your-orchestrator-url> \
     NAMESPACE=<your-namespace>
   ```
2. **Customize** labels and naming if needed (edit YAML files in `tekton/eventlistener/`)
3. **Test** with your orchestrator instance using `test-eventlistener.sh`
4. **Extend** pipeline with your specific requirements
