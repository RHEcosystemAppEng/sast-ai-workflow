...
- Circuit breakers: max iterations, duplicate detection, error recovery,
  stalled progress, guard rejection limit
"""
...
def route_next(
    ...
    *,
    ...
    made_progress = (
        ...
        or (
            current_unknowns < state.snapshot_unknowns_count
        )  # Resolved unknowns counts as progress
    )
    ...
    if last_tool_name != "evaluator":
        ...
        if made_progress:
            logger.info(
                f"[{state.issue_id}] Progress: claims "
                f"{state.snapshot_claims_count}→{current_claims}, "
    ...
    if state.no_progress_streak >= 3:
        logger.warning(
            f"[{state.issue_id}] Circuit breaker: " "Stalled progress (no_progress_streak)"
        )
        return "circuit_breaker"
