                        ...
                        messages.append(
                            HumanMessage(
                                content=(
                                    ...
                                    "You MUST respond with a JSON object with this "
                                    "EXACT structure:\n"
                                    "{\n"
                                    '  "analysis": "string - your investigation summary",\n'
                                    '  "claims": [ list of claim objects with claim_id, '
                                    "claim, confidence, evidence_ids ],\n"
                                    '  "evidence": [ list of evidence objects with '
                                    "evidence_id, path_or_identifier, excerpt, "
                                    "start_line, end_line, why_it_matters ],\n"
                                    '  "unknowns": [ list of unknown objects with '
                                    "unknown_id, question, priority, blocking ],\n"
                                    '  "next_tool": "string - tool name from: fetch_code, '
                                    'evaluator, list_files, read_file, search_codebase",\n'
                                    '  "tool_reasoning": "string - why you chose this '
                                    'tool",\n'
                                    '  "tool_parameters": { object - parameters for the '
                                    "tool }\n"
                                    "}\n\n"
                                    "CRITICAL: All 7 fields are REQUIRED. Do not omit "
                                    "any field. If a list is empty, use []. If you have "
                                    "no tool to call, use next_tool: 'evaluator'."
                                )
                            )
                        )
                ...
                messages.append(
                    HumanMessage(
                        content=(
                            ...
                            "Please ensure your JSON response is valid and "
...
async def agent_decision_node(
    ...
    if state.iteration_count >= max_iterations - 3:
        logger.warning(
            f"[{state.issue_id}] Approaching max iterations "
        ...
        proposed_verdict = (
            ...
            else ("TRUE_POSITIVE" if has_evidence else "NEEDS_REVIEW")
        )

        # Create forced evaluator reasoning
        forced_reasoning = ReasoningStateUpdate(
            analysis=(
                f"Investigation reached iteration "
                f"{state.iteration_count}/{max_iterations}. "
            ...
            next_tool="evaluator",
            tool_reasoning=(
                f"Forcing evaluation at iteration " f"{state.iteration_count}/{max_iterations}"
            ),
            tool_parameters={
                "analysis": (
                    f"Investigation concluded after " f"{state.iteration_count} iterations"
                ),
    ...
    available_tools = (
        format_tools_for_prompt(tools) if tools else "No tools documentation available."
    )
    ...
    formatted_content = template.safe_substitute(
        ...
        fetched_code=formatted_fetched_code,
    ...
    try:
        ...
        if reasoning is None:
            ...
            if (
                state.error_state.error_recovery_attempts >= 3
                or state.iteration_count >= 15
            ):
                logger.warning(
                    f"[{state.issue_id}] Too many error recovery attempts "
                    f"({state.error_state.error_recovery_attempts}) or "
                    f"iterations ({state.iteration_count}). "
                    "Forcing evaluator call to exit gracefully."
                )
        ...
        logger.info(
            f"[{state.issue_id}] Iteration {state.iteration_count}: Structured reasoning received"
        )
