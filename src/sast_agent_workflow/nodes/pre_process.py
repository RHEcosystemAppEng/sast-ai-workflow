...
def _create_default_analysis_response() -> AnalysisResponse:
    return AnalysisResponse(
        ...
        recommendations=[DEFAULT_FIELD_VALUE],
        instructions=[],
        evaluation=[DEFAULT_FIELD_VALUE],
        prompt=DEFAULT_FIELD_VALUE,
    )
...
async def pre_process(config: PreProcessConfig, builder: Builder):
    """Register the Pre_Process function."""

    logger.info("Initializing Pre_Process function...")

    async def _pre_process_fn(  # NOSONAR - async required by NAT framework
        empty_input: dict,
    ) -> SASTWorkflowTracker:
        """
        Pre-processing function for SAST workflow.
        ...
        tracker = SASTWorkflowTracker(
            issues=issues_dict, config=config, iteration_count=0, metrics={}
        )
        try:
            # Initialize the repo handler, just to download the repo if needed
            # TODO: Refactor this - factory creation should not perform
            #       side effects like downloading repos. Consider
            #       separating repo init/downloading from factory creation
