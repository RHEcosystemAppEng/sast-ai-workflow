...
Uses LangChain create_agent API with middleware pattern.
...
class ResearchAgentState(AgentState):
    ...
    Extends LangChain AgentState with domain-specific fields
...
# ============================================================================
# Middleware (LangChain create_agent Pattern)
# ============================================================================
...
# ============================================================================
# Research Node Constructor
# ============================================================================


def create_research_node(llm: BaseChatModel, tools: List[BaseTool]):
    """
    Create research node using LangChain create_agent with middleware.

    This uses:
    - LangChain create_agent with middleware pattern
    ...
    async def research(state: InvestigationState) -> InvestigationState:
        ...
        invoke_config = {
            ...
            "run_name": LANGFUSE_RESEARCH_TRACE_NAME,
            "type": "actor",
            "run_type": "agent",
        }
        ...
        try:
            ...
            logger.warning(
                ...
                f"({RESEARCH_AGENT_RECURSION_LIMIT} steps) "
            ...
            logger.error(
                f"[{state['issue_id']}] Research agent error: {e}",
                exc_info=True,
            )
...
def _handle_research_recursion_limit(
    ...
    if from_checkpointer:
        logger.info(
            f"[{state['issue_id']}] Preserved state via checkpointer: {len(fetched)} tool results, "
            f"{len(history)} tool calls"
        )
    gathered_code = _merge_gathered_code(state.get("gathered_code", ""), fetched)
    total_tool_calls = _count_tool_calls(state, messages)
...
def _get_preserved_state(
    ...
    try:
        ...
        if log_failure:
            logger.warning(
                f"Could not retrieve state from "
