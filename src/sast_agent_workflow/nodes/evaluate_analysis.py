...
class EvaluateAnalysisConfig(FunctionBaseConfig, name="evaluate_analysis"):
    ...
    description: str = Field(
        default=(
            "Evaluate analysis function that evaluates the results "
            "of SAST analysis and decides if analysis is final"
        ),
        description="Function description",
    )
    llm_name: str = Field(default="main_llm", description="LLM name to use for evaluation")


@register_function(
    config_type=EvaluateAnalysisConfig, framework_wrappers=[LLMFrameworkEnum.LANGCHAIN]
)
async def evaluate_analysis(config: EvaluateAnalysisConfig, builder: Builder):
    """
    ...
    async def _evaluate_analysis_fn(tracker: SASTWorkflowTracker) -> SASTWorkflowTracker:
        ...
        if tracker.iteration_count >= tracker.config.MAX_ANALYSIS_ITERATIONS:
            logger.info(
                f"Max analysis iterations reached: "
                f"{tracker.iteration_count} >= "
                f"{tracker.config.MAX_ANALYSIS_ITERATIONS}, "
                f"skipping evaluate_analysis node"
            )
            return tracker
        ...
        for issue_id, issue_data in tracker.issues.items():
            ...
            if (
                issue_data.analysis_response
                and issue_data.analysis_response.is_final == FinalStatus.FALSE.value
            ):
                non_final_issues += 1
        ...
        logger.info(
            f"Evaluate_Analysis node completed. "
...
def _process_single_issue_evaluation(
    issue_id: str, issue_data: PerIssueData, issue_analysis_service: IssueAnalysisService, llm
) -> None:
    """
    ...
    try:
        ...
        logger.info(
            f"{issue_id}: Evaluation completed. is_final: {recommendations_response.is_final}"
        )
