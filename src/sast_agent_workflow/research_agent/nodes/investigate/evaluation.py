...
from ...core import build_evaluation_prompt
...
def create_evaluation_node(llm: BaseChatModel, config: Config):
    ...
    async def evaluate(state: InvestigationState) -> InvestigationState:
        ...
        # Determine completion (don't check max_iterations or rejection_streak here - router handles it)
        ...
        if is_complete:
            ...
            logger.info(
                f"[{issue_id}] Needs more research (iteration {state['iteration']}/{state['max_iterations']}, "
