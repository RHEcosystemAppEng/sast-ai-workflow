    ...
    async def investigate(state: SASTWorkflowTracker) -> SASTWorkflowTracker:
        ...
        try:
            ...
            if ground_truth_verdicts:
                logger.info(f"Loaded ground truth for {len(ground_truth_verdicts)} issues for Langfuse scoring")
        ...
        for issue_id, per_issue in issues_to_investigate.items():
            ...
            try:
                ...
                logger.debug(f"[{issue_id}] Extracted {len(initial_code) if initial_code else 0} files from trace")
                ...
                if not code_context:
                    code_context = "(No code could be automatically extracted from the trace. The agent will need to fetch code based on the trace information above.)"
                ...
                if initial_code:
                    ...
                    logger.info(
                        ...
                        f"({len(gathered_code_initial)} chars): {list(initial_code.keys())}"
                    )
                else:
...
{per_issue.issue.trace}"""
                ...
                logger.info(f"{issue_id}: {verdict} (confidence: {result['confidence']}, iterations: {iterations})")
...
def _add_langfuse_scores(
    ...
    try:
        ...
        tool_call_count = sum(
            1 for msg in result.get('research_messages', [])
        ...
        if ground_truth_verdicts and issue_id in ground_truth_verdicts:
            ...
            if ground_truth_verdicts is None:
                logger.debug(f"[{issue_id}] No ground truth available - skipping verdict correctness score")
            else:
                logger.debug(f"[{issue_id}] No ground truth for this issue - skipping verdict correctness score")
