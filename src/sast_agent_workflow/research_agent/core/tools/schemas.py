...
from ..constants import DEFAULT_CONTEXT_LINES, DEFAULT_MAX_SEARCH_RESULTS
...
class SearchCodebaseInput(BaseModel):
    ...
    file_pattern: Optional[str] = Field(
        default="*.c",
        description=(
            "File pattern to search (e.g., '*.c', '*.h', '*.py', '*.java'). "
            "Default: '*.c'"
        )
    )
...
class ReadFileInput(BaseModel):
    ...
    file_path: str = Field(
        description="Path to file relative to repo root (e.g., 'src/main.c')"
    )
