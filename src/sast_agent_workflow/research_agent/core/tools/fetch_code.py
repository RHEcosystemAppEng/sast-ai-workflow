...
from ..constants import DEFAULT_CONTEXT_LINES, MAX_FUNCTION_SCAN_LINES
...
def create_fetch_code_tool(repo_handler, repo_path: Path) -> StructuredTool:
    ...
    def _fetch_code(
        ...
        """
        logger.info(f"fetch_code: {file_path}" + (f" function={function_name}" if function_name else ""))
        
        # Try sophisticated approach first (for functions)
        if function_name:
            result = _try_sophisticated_extraction(
                repo_handler, file_path, function_name
            )
            if result:
                return result
        
        # Fallback to simple file reading
        return _simple_file_extraction(
            repo_path, file_path, function_name, context_lines
        )
    ...
    return StructuredTool.from_function(
        ...
        description=(
            ...
            "Parameters:\n"
            "- file_path: Relative path to file without the repository name suffix (e.g., 'src/main.c', 'lib/auth.c')\n"
            "- function_name: Optional function name to extract (e.g., 'validate_input')\n"
            f"- context_lines: Lines of context around function (default: {DEFAULT_CONTEXT_LINES})\n\n"
            "Examples:\n"
...
=== End of {function_name} ==="""
        ...
        logger.warning(
            f"Sophisticated extraction error for {function_name}: {e}, "
    ...
    try:
        ...
        if not full_path.exists():
            return (
                ...
                f"1. Use search_codebase(pattern='{function_name or file_path}', file_pattern='*.c')\n"
                f"2. Use list_directory(dir_path='{Path(file_path).parent}') to explore the directory"
            )
        ...
        return _extract_function_simple(
            lines, file_path, function_name, context_lines
        )
...
def _extract_function_simple(
    ...
    if function_start is not None:
        ...
        return (
            ...
            f"Suggested recovery:\n"
            f"1. Use search_codebase(pattern='{function_name}', file_pattern='*.c') "
            f"to find where it's defined\n"
