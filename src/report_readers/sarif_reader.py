...
class SarifReportReader(BaseReportReader):
    ...
    def _is_sarif_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if not version or version not in self.SUPPORTED_SARIF_VERSIONS:
                logger.warning(f"Unsupported SARIF version: {version}. Supported versions: {self.SUPPORTED_SARIF_VERSIONS}")
        ...
        for run_index, run in enumerate(runs):
            ...
            for result_index, result in enumerate(results):
                try:
                    ...
                    logger.warning(f"Error processing SARIF result {result_index} in run {run_index}: {e}")
                    continue
    ...
    def _extract_issue_type(self, result: Dict[str, Any], rule: Dict[str, Any], rule_id: str) -> str:
        """
        ...
        if rule:
            ...
            for desc_key in ["shortDescription", "fullDescription"]:
                ...
                if isinstance(desc_obj, dict) and "text" in desc_obj:
                    cve_match = re.search(r'(CWE-\d+|CVE-\d{4}-\d+)', desc_obj["text"], re.IGNORECASE)
                    if cve_match:
                        cve = cve_match.group(1).upper()
                        cve_info["cve"] = cve
                        cve_info["cve_link"] = self._generate_cve_link(cve)
    ...
    def _should_skip_by_kinds(self, kinds: List[str]) -> bool:
        ...
        important_kinds = {
            'acquire', 'release',      # Resource management
            'memory', 'resource',      # Memory/resource operations  
            'taint',                   # Taint analysis
            'danger', 'caution',       # Security warnings
        ...
        control_flow_kinds = {
            'branch', 'true', 'false', # Branch decisions
            'enter', 'exit',           # Scope entry/exit
    ...
    def _extract_step_type(self, location: Dict[str, Any]) -> str:
        ...
        if kinds:
            ...
            priority_kinds = ['memory', 'resource', 'taint', 'danger', 'acquire', 'release', 'value']
