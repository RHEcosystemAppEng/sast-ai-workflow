...
def categorize_issues_by_status(issues: dict[str, PerIssueData]) -> Counter:
    ...
    Returns:
        ...
        - 'known_non_issues': Issues containing known non-issue justification
        - 'issues': Issues marked as real security issues
        - 'non_issues': Issues marked as non-issues (false alarms)
    ...
    if counter[TOTAL_ISSUES] == 0:
        (
            counter[NO_ANALYSIS_RESPONSE],
            counter[NON_FINAL_ISSUES],
            counter[FINAL_ISSUES],
            counter[KNOWN_NON_ISSUES],
            counter[ISSUES],
            counter[NON_ISSUES],
            counter[NEEDS_SECOND_ANALYSIS],
        ) = (0, 0, 0, 0, 0, 0, 0)
        return counter
    ...
    for issue_data in issues.values():
        ...
        # Count known non-issues
        if (
            KNOWN_NON_ISSUES_SHORT_JUSTIFICATION
            in issue_data.analysis_response.short_justifications
        ):
            counter[KNOWN_NON_ISSUES] += 1

        # Count issues/non-issues
        if issue_data.analysis_response.is_issue():
            counter[ISSUES] += 1
        else:
            counter[NON_ISSUES] += 1
