...
def convert_tracker_to_summary_data(
    tracker: SASTWorkflowTracker, include_non_final: bool = True, filter_failed: bool = True
) -> List[Tuple[Issue, SummaryInfo]]:
    """
    Convert SASTWorkflowTracker to summary_data format expected by
    ExcelWriter and EvaluationSummary.
...
def get_linear_edges() -> List[Tuple[str, str]]:
    ...
    return [
        ...
        (WorkflowNode.FILTER.value, WorkflowNode.INVESTIGATE.value),
        (WorkflowNode.INVESTIGATE.value, WorkflowNode.SUMMARIZE_JUSTIFICATIONS.value),
