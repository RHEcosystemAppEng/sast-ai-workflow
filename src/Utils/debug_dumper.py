...
class DebugDumper:
    ...
    def dump_checkpoint(self, checkpoint_name: str, data: Any, issue_id: str = None) -> str:
        """
        ...
        # Create filename
        if issue_id:
            filename = f"{self.dump_counter:03d}_{checkpoint_name}_{issue_id}_{self.session_id}.json"
        else:
            filename = f"{self.dump_counter:03d}_{checkpoint_name}_{self.session_id}.json"
    ...
    def dump_llm_investigation(self, issue_id: str, llm_response: Any, critique_response: str, 
                             context: str, iteration: int = 1) -> str:
        """Dump LLM investigation results"""
        checkpoint_name = f"llm_investigate_iteration_{iteration}"
        return self.dump_checkpoint(checkpoint_name, {
