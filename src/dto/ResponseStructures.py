...
class JudgeLLMResponse(BaseModel):
    """
    Structured response for analyzing and classifying issues,
...
class InstructionResponse(BaseModel):
    expression_name: str = Field(
        "The exact name of the function or macro that requires further inspection."
    )
...
class RecommendationsResponse(BaseModel):
    is_final: str = Field(
        description="An indicator if the model response is final or additional query is needed. \
    ...
    instructions: List[InstructionResponse] = Field(
        ...
        description="list of InstructionResponse instances, \
            each instruction represents an expression to retrieve from the source code",
    )
...
class JustificationsSummary(BaseModel):
    ...
    short_justifications: str = Field(
        description="A clear, concise summary of the justification \
            written in an engineer-style tone, highlighting the most impactful point."
    )
...
class EvaluationResponse(BaseModel):
    ...
    critique_result: str = Field(
        description="The result of the investigation. \
            Possible values are 'FALSE POSITIVE' or 'NOT A FALSE POSITIVE'."
    )
    justifications: List[str] = Field(
        description="A list of reasons explaining the conclusion of the investigation."
    )
