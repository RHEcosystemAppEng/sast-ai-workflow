...
from dto.ResponseStructures import EvaluationResponse, FilterResponse
...
class LLMService:
    ...
    def __init__(self, config: Config, main_llm=None, embedding_llm=None, critique_llm=None):
        ...
        self.issue_analysis_service = IssueAnalysisService(
            config=config, vector_service=self.vector_service
        )
    ...
    def filter_known_error(self, issue: Issue, similar_findings_context: str) -> FilterResponse:
        ...
        """
        return self.issue_analysis_service.filter_known_issues_from_context(
            issue, similar_findings_context, self.main_llm
        )

    def investigate_issue(
        self, context: str, issue: Issue
        ...
        Returns:
            tuple: A tuple containing:
                - llm_analysis_response (AnalysisResponse): A structured response with the analysis
                  result.
                - critique_response (EvaluationResponse): The response of the critique model, if
                  applicable.
        """
